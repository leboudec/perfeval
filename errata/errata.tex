...
%
%
\includegraphics[scale=0.1]{../book/pictures/artwork/scouacMesureOrdi.pdf}}
...
\item Page 27, after Eq (2.7): $\mbox{CoV}$ and $\cor{\mbox{MAD}}{\mbox{gap}}$ converge to
their theoretical counterparts:
 \bearn
  & \dots &\\
 \cor{\mbox{MAD}}{\mbox{gap}} & \to & \mbox{ gap }_{\mbox{th}}=
    \frac{\int_0^{\infty}\abs{x-\mu} f(x) dx}{2 \mu}
 \eearn [Ludovic Thomas]
 \item Section 2.1.4, paragraph \textsc{Lorenz Curve} rewritten: equation (2.10) was incorrect and is fixed [Ludovic Thomas]. The proof of the property that the Lorenz Curve is below the diagonal is added.

\item Just before Eq (2.14): 
Its definition is similar to the Lorenz
curve gap, with the mean \cor{average}{absolute} deviation replaced by ... [Ludovic Thomas]
...
\item Page 57, Example 2.7: let $T^r=\cor{\frac{i=1}{n}X^r_i}{\frac1n\sum_{i=1}^{n}X^r_i}$ [Ludovic Thomas]
\item Page 59, Example 2.8: In the data of Example~\cor{2.8}{2.6} 
\item Page 59, Caption of Figure 2.15:  methods discussed in Example~\cor{2.8}{2.6} [Ludovic Thomas]

\item Page 65, Question 2.8.1: From the tables in Chapter A and Theorem 2.8.5 we obtain:
 (confidence interval for median, prediction interval):
 $n=9$: $[x_{(2)},x_{(\cor{9}{8})}]$, impossible;
 $n=39$: $[x_{(13)},x_{(27)}]$, $[x_{(1)},x_{(39)}]$;
 $n=99$: $[x_{(39)},x_{(61)}]$, $[x_{(2)},x_{(9\cor{7}{8})}]$.
 The confidence interval is always smaller than the prediction interval. [Ludovic Thomas]
 \end{itemize}
 \subsection*{Chapter 3}
 \begin{itemize}
    \item Page 73, after Eq (3.6), added: Note that the parameter $\lambda$ is a scale parameter, more precisely, if $X
\sim \mbox{
 Laplace}(\lambda)$ we can write $X=\frac1{\lambda} Y$ for some $Y
\sim \mbox{
 Laplace}(1)$.
...
\item Page 78, footnote (answer to Question 3.2.2). Yes, since the confidence
 interval for  \cor{$d$}{$b$ [resp. $d$]} is entirely positive [resp. negative] [Ludovic Thomas].
\item Page 81, Example 3.9, added : (since $\lambda$ is a scale parameter for the Laplace distribution, it is sufficient to use $\lambda=1$)
...
to $1-\cor{e^{\lp  x ^c \rp}}{e^{-\lp  x ^c \rp}}$ [Julien Harbulot].
...
The idea is to keep a running estimate \cor{estimate}{} $\hat{m}_t$... [Roman Rudnik].
 ...
 \item Page 218, \textbf{Virtual System} The $n$th customer arrives at
        time $T_n+r_0$. If $\cor{T}{R}_n< r_0$, this customer leaves
