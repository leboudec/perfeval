...
It can be used to model error terms that have a heavier tail
 ...
 \ee
%
Note that the parameter $\lambda$ is a scale parameter, more precisely, if $X
\sim \mbox{
 Laplace}(\lambda)$ we can write $X=\frac1{\lambda} Y$ for some $Y
\sim \mbox{
 Laplace}(1)$. The next theorem explains what we do when we fit
...
similar and is not shown).
 ...
 interval for $b$ [resp. $d$] is entirely positive [resp. negative].}
\label{ex-joe-jjjj}
\end{ex}
...
\begin{figure*}[htb!]
  \begin{center}
    \includegraphics[angle=0,scale=0.6]{hw54a}
...
\begin{shadethm} Consider the model in Definition~\ref{def:ls:lin}; let $\vec{y}$ be
...
Laplace distribution with rate $\lambda=1$ (since $\lambda$ is a scale parameter for the Laplace distribution, it is sufficient to use $\lambda=1$). A
