...
0\leq L(p) \leq p, \mfa p\in [0,1]
  \ee
because a convex function lies below its chords. Furthermore, if $L(p)=p$ for some $p\in (0,1)$ then $L(p)=p$ for all $p\in [0,1]$; this is because a non constant convex function such as $L(p)-p$ can have at most two zeroes. Such a case occurs only when all $x_i$'s are equal (to see why, consider that the $i$th slope of $L$, which is $\frac{x_{(i)}}{nm}$, must be equal to $1$ for all $i$). 
