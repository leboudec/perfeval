...
$(p_i, \ell_i)$ with
 ...
 p_i =  \frac{i}{n}
 \\
 \ell_i  =  \frac{x_{(1)} + ...+ x_{(i)} }{nm}
 \label{eq-calcul-lorenz}
 }\ee
where $x_{(1)} \leq x_{(2)}\leq \ldots \leq x_{(n)}$ is obtained by sorting $x_i$ in increasing order and $m = \frac{x_{1} + ...+ x_{n} }{n}$ is the sample mean.
...
$[0,1]$ onto $[0,1]$, monotone non decreasing, piecewise linear, with $L(0)= 0$ and $L(1)=1$. It is also convex because the slopes of the segments, $x_{(i)}$, constitute an increasing sequence.
...
%\label{eq-calcul-lorenz}\ee 
% Note that $0 \leq l_i \leq
% \frac{i}{n}$, thus
%   \be
% 0\leq L(p) \leq p
%   \ee
% and the Lorenz curve is always between the
% lines of perfect equality and perfect inequality.

% We claim that, for all $i$, $l_i \leq
%  \frac{i}{n}$ and that if there is equality for some $i$, then all $x_i$'s must be equal. To see why, rewrite $l_i$ as
%  \ben
%  l_i= \frac{x_{(1)} + ...+ x_{(i)} }{x_{(1)} + ...+ x_{(n)}}
%  =\frac{1}{1+\frac{x_{(i+1)} + ...+ x_{(n)}}{x_{(1)} + ...+ x_{(i)}}}
%  \een
%  Now $x_{(1)} + ...+ x_{(i)}\leq i x_{(i)}$ and $x_{(i+1)} + ...+ x_{(n)}\geq (n-i) x_{(i)}$ hence 
% \ben
% l_i \leq \frac{1}{1+\frac{(n-i)x_{(i)}}{ix_{(i)}}}=\frac{i}n
% \een
% Furthermore, if we have equality in the previous equation, then we must have $x_{(1)} + ...+ x_{(i)}= i x_{(i)}$ and $x_{(i+1)} + ...+ x_{(n)}= (n-i) x_{(i)}$. It follows that $x_{(1)} = ...= x_{(i)}$ and $x_{(i+1)} = ...= x_{(n)}=  x_{(i)}$ hence all $x_{(i)}$'s are equal and so are all $x_i$'s, which proves the claim. 

Note that $L$ is convex and $L(0)=L(1)=0$, it follows that 
  \be
0\leq L(p) \leq p, \mfa p\in [0,1]
  \ee
because a convex function such as $L(p)-p$ lies below its chords. Furthermore, if $L(p)=p$ for some $p\in (0,1)$ then $L(p)=p$ for all $p\in [0,1]$. To see why, assume that $L(p)-p=0$ for some $p\in (0,1)$ and $L(q)-q<0$ for some other $q\in (0,1)$; assume $p<q$; then $L(p)-p$ is below the chord $(0,0)-(q,L(q)-q)$ hence $L(p)-p<0$, which contradicts $L(p)-p=0$; and similarly if $p>q$. Furthermore, the case $L(p)=p$ for all $p\in [0,1]$ occurs only when all $x_i$'s are equal. To see why, consider that the $i$th slope of $L$, which is $\frac{x_{(i)}}{m}$, must be equal to $1$ for all $i$. 

In summary the Lorenz curve always lies between the
lines of perfect equality and perfect inequality, and it never meets the line of perfect equality except when all $x_i$'s are equal.
...
curve gap, with the mean absolute deviation replaced by the
\nt{Mean Difference}:
  \be
  \mbox{MD} = \frac{1}{n(n-1)}\sum_{i, j}\abs{x_i-x_j}
  \ee
...
\mq{q-conf-kasdj}{Can the confidence interval for the mean in
\thref{theo-conf-normal} be derived solely from the sample size $n$ and the value of the point estimator of the mean $\hat{\mu}_n$? Can the confidence interval for the variance in
\thref{theo-conf-normal} be derived solely from the sample size $n$ and the value of the point estimator of the variance $\hat{\sigma}_n$?}{1. No, we also need the value of $\hat{\sigma}_n$. 2 Yes.}
...
method in \thref{theo-conf-ln} and with the bootstrap method (right).
...
data, which we assume to be an estimate of an attribute $t_F$ of the 
unknown distribution, and for which we want to find a confidence interval (in this
...
\sref{sec-var}:
 ...
 \een
 and $t_F$ is the theoretical Lorenz curve gap as defined in the text after \eref{eq-def-l-th}.

%Here, we replace the confidence interval given in Definition~\ref{def:ci} by a prediction interval for the statistic $t(\vec{X})$, where $\vec{X}=(X_1,...,X_n)$ is an iid sample drawn from the distribution $F$. 

Assume for a second that an oracle would give us the distribution of $t(\vec{X})-t_F$ when $\vec{X}=(X_1,...,X_n)$ is an iid sample drawn from the distribution $F$. Then let $\alpha_F$ [resp. $\beta_F$] be the $\frac{1-\gamma}{2}$ [resp. $\frac{1+\gamma}{2}$] percentile of the distribution of $t(\vec{X})-t_F$, where $\gamma$ is a given confidence level (e.g. $\gamma =0.95$). We have thus
\ben
\P(-\alpha_F \leq t(\vec{X})-t_F\leq \beta_F)= \gamma
\een 
It follows that a confidence interval for $t_F$ at level $\gamma$ would be given by
\ben
[t(\vec{x})-\beta_F, \;t(\vec{x})+\alpha_F]
\een

With the \nt{bootstrap method}, we replace the unknown distribution $F$ by the empirical distribution of the sample
$\vec{x}=(x_1,...,x_n)$. It is justified by the Glivenko-Cantelli
theorem which says that the ECDF converges with probability 1
to the true CDF $F()$ when $n$ gets large. Specifically, the method draws an iid sample $\vec{X'}=(X_1',...,X_n')$ from the empirical distribution of the sample
$\vec{x}$; this means drawing $n$ times from the list
$(x_1,...,x_n)$ \imp{with replacement}. Next we compute some bounds $\alpha_{\vec{x}}$ and $\beta_{\vec{x}}$ such that
\ben
\P(-\alpha_{\vec{x}} \leq t(\vec{X}')-t(\vec{x})\leq \beta_{\vec{x}})= \gamma
\een  
In the above, $\vec{X}'$ is random and $\vec{x}$ is not. Then the bootstrap method takes $\alpha_{\vec{x}}, \beta_{\vec{x}}, t(\vec{x})$ as approximations for $\alpha_F, \beta_F, t_F$; the resulting approximate confidence interval for $t_F$ is
\ben
[t(\vec{x})-\beta_{\vec{x}}, \;t(\vec{x})+\alpha_{\vec{x}}]
\een
Observe that the lower bound $t(\vec{x})-\beta_{\vec{x}}$ is a $\frac{1-\gamma}{2}-$percentile of the distribution of $\vec{X}'$ (and similarly the upper bound $t(\vec{x})+\alpha_{\vec{x}}$ is a $\frac{1+\gamma}{2}-$percentile of the distribution of $\vec{X}'$). The method estimates them using percentiles of many independent replications of $\vec{X}'$.  
...
$(x_1,...,x_n)$ with replacement. For example, if all
...
let $T^r=\frac1n\sum_{i=1}^{n}X^r_i$. $T^r$ is the $r$th
...
\center
 ...
 \subfigure[(Prediction Intervals)]{\Ifignc{pif13}{0.4}{0.3}}
  ...
  outlier,
   computed with the three alternative methods discussed in \exref{ex-conf-ft1a}: (1) order statistics
...
In fact in the data of \exref{ex-conf-ft1a} there is one very
...
\mq{q-conf-aslasliq}
 ...
 $n=9$: $[x_{(2)},x_{(8)}]$, impossible;
 $n=39$: $[x_{(13)},x_{(27)}]$, $[x_{(1)},x_{(39)}]$;
 $n=99$: $[x_{(39)},x_{(61)}]$, $[x_{(2)},x_{(98)}]$.
